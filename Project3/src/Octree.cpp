...
void Octree::create(const ofMesh & geo, int numLevels) {
	...
	//
	float octTreeBuildTime = ofGetElapsedTimeMillis();
	...
	cout << "Time to build Octree: " << ofGetElapsedTimeMillis() - octTreeBuildTime << " milliseconds" << endl;
}
...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	for (int i = 0; i < boxList.size(); i++) {
		TreeNode child;
		child.box = boxList[i];
		int points = getMeshPointsInBox(mesh,node.points,child.box,child.points);
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	...
	//float searchTime = ofGetElapsedTimeMillis();
	...
	//cout << "Time to search data with the ray intersection selection : " << ofGetElapsedTimeMillis() - searchTime << " milliseconds" << endl;
}
